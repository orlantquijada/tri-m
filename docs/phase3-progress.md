...
- [x] **I3** — Distributor performance report (per-distributor aggregates, date-range filter)
...
- 2026-05-18 — I3 — Three parallel DB queries merged in JS: stats (date-independent; distributors→customers→receivables), collected (date-filtered non-voided payments→receivables), overdue (EXISTS-based to avoid double-count from schedule join). collectionRatePct computed in JS, rounded to 1 decimal. Client-side sorting via useState; color thresholds ≥80% green / 50–79% yellow / <50% red. Manually patched routeTree.gen.ts for /reports/ route (same pattern as I1/H2).
